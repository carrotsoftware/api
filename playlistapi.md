...
1. В ответ вы получите сообщение-уведомление вида:
    ...
    - `PlaylistGroupId` - идентификатор родительской группы изменённой группы плейлистов. Тип `Guid`.    Необходим для [обновления содержимого родительской группы](#Получение-списка-плейлистов).
...
1. В ответ вы получите сообщение-уведомление вида:
    ...
    - `PlaylistGroupId` - идентификатор изменённой группы плейлистов, из которой удалена группа. Тип `Guid`. Необходим для [обновления содержимого группы](#Получение-списка-плейлистов).
...
1. [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:
    ...
    > Выгрузка шаблона произойдёт только если будут выгружены все события, относящиеся к шаблону.
