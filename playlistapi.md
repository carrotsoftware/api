...
1. Изменение группы плейлистов. Сообщение имеет вид:
    ...
    - `PlaylistGroupId` - идентификатор группы плейлистов, в которую добавлен новый     плейлист. Тип `Guid`. Необходим для [обновления содержимого группы]   (#Получение-списка-плейлистов).
    - `ParentId` - идентификатор родительской группы изменённой группы плейлистов.    Тип `Guid`. Необходим для [обновления содержимого родительской группы]  (#Получение-списка-плейлистов).
...
1. Изменение плейлиста. Сообщение имеет вид:
    ...
    - `PlaylistGroupId` - идентификатор группы, к которой относится плейлист. Тип     `Guid`. Необходим для [обновления содержимого родительской группы]    (#Получение-списка-плейлистов).
...
1. Для этого [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:
    ...
    - `Playlist` - структура плейлиста.
      ...
      - `PlaylistGroupId` - идентификатор существующей группы плейлистов, к которой будет     относиться новый плейлист. Тип `Guid`. Выбирается из идентификаторов групп, полученных при    [запросе списка плейлистов](#Получение-списка-плейлистов).
...
1. В ответ вы получите сообщение-уведомление вида:
    ...
    - `PlaylistGroupId` - идентификатор группы плейлистов, в которой плейлист был изменен. Тип    `Guid`. Необходим для [обновления содержимого группы](#Получение-списка-плейлистов).
...
1. В ответ вы получите сообщения-уведомление вида (два сообщения в случае `DeleteOriginal` = `true`):
    ...
    - `PlaylistGroupId` - идентификатор изменённой группы плейлистов, в которую добавлена копия     плейлиста / из которой удален плейлист (если `DeleteOriginal` = `true`). Тип `Guid`. Необходим    для [обновления содержимого группы](#Получение-списка-плейлистов).
    - `ParentId` - идентификатор родительской группы изменённой группы плейлистов. Тип `Guid`.    Необходим для [обновления содержимого родительской группы](#Получение-списка-плейлистов).
...
1. В ответ вы получите два сообщения-уведомления вида:
    ...
    - `PlaylistGroupId` - идентификатор изменённой группы плейлистов из которой удален плейлист.    Тип `Guid`. Необходим для [обновления содержимого группы](#Получение-списка-плейлистов).
...
1. В ответ вы получите сообщение-уведомление вида:
    ...
    - `PlaylistGroupId` - идентификатор изменённой группы плейлистов, в которую добавлена копия плейлиста / из которой удален плейлист. Тип `Guid`. Необходим для [обновления содержимого группы](#Получение-списка-плейлистов).
...
1. Для этого [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:
    ...
    ```xml
    <Command CmdGroup="Playlists" CmdName="AddScenario" MessageId="79">
      <Scenario>
        <Id>ac8fdff9-a841-4714-86d5-3e5b490d6d32</Id>
        <Name>New Story</Name>
        <Created>01.01.0001 0:00:00</Created>
        <Changed>01.01.0001 0:00:00</Changed>
        <IsSystem>false</IsSystem>
    ...
    - `Id` - идентификатор новой истории. Тип `Guid`. Генерируется на стороне клиента.
    - `Name` - желаемое имя истории. Тип `String`.
    - `PlaylistId` - идентификатор плейлиста, куда будет добавлена новая история. Тип `Guid`.
...
1. В ответ вы получите сообщение-уведомление вида:
    ...
    - `PlaylistId` - идентификатор плейлиста, куда была добавлена история. Тип `Guid`. Необходим для [обновления содержимого плейлиста](#Получение-полного-содержимого-плейлиста).
    - `PlaylistGroupId` - идентификатор группы плейлистов, в которой плейлист был изменен. Тип    `Guid`. Необходим для [обновления содержимого группы](#Получение-списка-плейлистов).

1. [Обновите содержимое плейлиста](#Получение-полного-содержимого-плейлиста).

## Изменение имени истории

1. Для этого [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="UpdateScenario" MessageId="30">
      <Scenario>
        <Id>625e4c11-294a-4eb7-8ccf-58003582629b</Id>
        <Name>New Story 4</Name>
        <Created>28.10.2021 16:44:30</Created>
        <Changed>28.10.2021 16:51:01</Changed>
        <IsSystem>false</IsSystem>
        <PlaylistId>fb36f71e-8ab5-4c10-a82d-1eba0df9659b</PlaylistId>
        <Duration>0</Duration>
        <CurrentTime>0</CurrentTime>
        <IsDefault>false</IsDefault>
        <EventList>
          <EventId>220dc064-cba2-4be4-ab40-e1da7f65ad14</EventId>
        </EventList>
      </Scenario>
    </Command>
    ```
    ...
    - `Id` - идентификатор истории, которую нужно изменить. Тип `Guid`.
    - `Name` - желаемое имя истории. Тип `String`.
...
1. В ответ вы получите сообщение-уведомление вида:
    ...
    <Command CmdGroup="Notifications" CmdName="ScenarioChanged">
      <PlaylistId>fb36f71e-8ab5-4c10-a82d-1eba0df9659b</PlaylistId>
      <ScenarioId>625e4c11-294a-4eb7-8ccf-58003582629b</ScenarioId>
    </Command>
    ```

    где:

    - `PlaylistId` - идентификатор плейлиста, где была изменена история. Тип `Guid`. Необходим для [обновления содержимого плейлиста](#Получение-полного-содержимого-плейлиста).
    - `ScenarioId` - идентификатор изменённой истории. Тип `Guid`. Необходим для [обновления содержимого истории](#Получение-содержимого-истории).

1. [Обновите содержимое истории](#Получение-содержимого-истории).
...
1. Для этого [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:
    ...
    ```xml
    <Command CmdGroup="Playlists" CmdName="DeleteScenario" MessageId="12">
      <ScenarioId>625e4c11-294a-4eb7-8ccf-58003582629b</ScenarioId>
    </Command>
    ```
    ...
    - `ScenarioId` - идентификатор истории, которую нужно удалить. Тип `Guid`.
...
1. [Выберите шаблон](#Получение-списка-шаблонов), на основании которого нужно создать элемент.

1. [Выберите плейлист](#Получение-списка-плейлистов), в котором будет создан элемент.

1. [Выберите историю](#Получение-списка-историй), в которой будет создан элемент.

1. [По соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="AddEvent" MessageId="25">
      <Event>
        <Id>dae0a929-9a57-4e4b-985d-2eb3c04caf3a</Id>
        <Name>BreakingNews_4</Name>
        <Created>01.01.0001 0:00:00</Created>
        <Changed>01.01.0001 0:00:00</Changed>
        <IsSystem>false</IsSystem>
        <PlaylistId>fb36f71e-8ab5-4c10-a82d-1eba0df9659b</PlaylistId>
        <ScenarioId>c32ef9f8-1dfc-45fd-a756-739d04d26ff8</ScenarioId>
        <TemplateId>d146a643-e0f9-44d4-b1a3-c1404d66bf0d</TemplateId>
        <ContentId>f802037b-ccd3-497f-86d1-558ba8fdd57f</ContentId>
        <Duration>0</Duration>
        <CurrentTime>0</CurrentTime>
        <StatusInt>0</StatusInt>
        <TemplateTypeInt>1</TemplateTypeInt>
        ...
        <Comment />
        <Variables>
          <Variable Id="0#13#12//&quot;CARROT ENGINE FLYING HIGH&quot;//&quot;Text&quot;//&quot; Source Text&quot;" Name="SubTitle" Type="Text" Value="" FieldId="59f4ba7d-9f73-4370-9e17-47bff562512d" ResetMedia="true" UseTimecode="false" Loop="false" Locked="false" DefaultValue="" MinValue="" MaxValue="" UseDataVars="false">
            <EditableFieldSettings>
              <FieldTypeInt>0</FieldTypeInt>
              <IsStill>true</IsStill>
              <NativeFrameRate>50</NativeFrameRate>
          ...
          <Variable Id="0#13#13//&quot;BREAKING NEWS&quot;//&quot;Text&quot;//&quot;Source Text&quot;" Name="Title" Type="Text" Value="" FieldId="a9bdab6c-bd0b-4546-845d-4b9679bff7e3" ResetMedia="true" UseTimecode="false" Loop="false" Locked="false" DefaultValue="" MinValue="" MaxValue="" UseDataVars="false">
            <EditableFieldSettings>
              <FieldTypeInt>0</FieldTypeInt>
              <IsStill>true</IsStill>
              <NativeFrameRate>50</NativeFrameRate>
    ...
    - `Id` - идентификатор нового элемента. Тип `Guid`. Генерируется на стороне клиента.
    - `Name` - желаемое имя элемента.
    - `PlaylistId` - идентификатор плейлиста, в котором будет создан элемент. Тип `Guid`.
    - `ScenarioId` - идентификатор истории, в которой будет создан элемент. Тип `Guid`.
    - `TemplateId` - идентификатор шаблона, состояние которого будет вызывать элемент. Тип `Guid`.
    - `ContentId` - идентификатор контейнера, в котором будет проигрываться шаблон. Тип `Guid`. ([содержится в структуре шаблона](#Получение-списка-шаблонов))
    - `TemplateTypeInt` - тип шаблона. Тип `Int`. ([содержится в структуре шаблона](#Получение-списка-шаблонов))
    - `State` - вызываемое состояние шаблона. Тип `String`. ([содержится в структуре шаблона](#Получение-списка-шаблонов))
    - `Variables` - список переменных шаблона.
      - `Variable` - конкретная переменная шаблона. Содержание атрибутов `Id`,`Name`,`Type`,`FieldId`,`DefaultValue`,`UseDataVars` идентично переменной `LinkVar` [в структуре шаблона](#Получение-списка-шаблонов)
...
1. [По соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:
    ...
    ```xml
    <Command CmdGroup="Playlists" CmdName="UpdateEvent" MessageId="50">
      <Event>
        <Id>dae0a929-9a57-4e4b-985d-2eb3c04caf3a</Id>
        <Name>BreakingNews_4</Name>
        <Created>28.10.2021 17:39:47</Created>
        <Changed>28.10.2021 17:41:56</Changed>
        <IsSystem>false</IsSystem>
        <PlaylistId>fb36f71e-8ab5-4c10-a82d-1eba0df9659b</PlaylistId>
        <ScenarioId>c32ef9f8-1dfc-45fd-a756-739d04d26ff8</ScenarioId>
        <TemplateId>d146a643-e0f9-44d4-b1a3-c1404d66bf0d</TemplateId>
        ...
        <Comment></Comment>
        <Variables>
          <Variable Id="0#13#12//&quot;CARROT ENGINE FLYING HIGH&quot;//&quot;Text&quot;//&quot; Source Text&quot;" Name="SubTitle" Type="Text" Value="4564" FieldId="59f4ba7d-9f73-4370-9e17-47bff562512d" ResetMedia="true" UseTimecode="false" Loop="false" Locked="false" DefaultValue="" MinValue="" MaxValue="" UseDataVars="false">
            <EditableFieldSettings>
              <FieldTypeInt>0</FieldTypeInt>
              <IsStill>true</IsStill>
              <NativeFrameRate>50</NativeFrameRate>
          ...
          <Variable Id="0#13#13//&quot;BREAKING NEWS&quot;//&quot;Text&quot;//&quot;Source Text& quot;" Name="Title" Type="Text" Value="9890"    FieldId="a9bdab6c-bd0b-4546-845d-4b9679bff7e3" ResetMedia="true" UseTimecode="false" Loop="false" Locked="false" DefaultValue="" MinValue="" MaxValue="" UseDataVars="false">
            <EditableFieldSettings>
              <FieldTypeInt>0</FieldTypeInt>
              <IsStill>true</IsStill>
              <NativeFrameRate>50</NativeFrameRate>
    ...
    - `Id` - идентификатор элемента, который нужно изменить. Тип `Guid`.
    - `Name` - желаемое имя элемента.
    - `State` - вызываемое состояние шаблона. Тип `String`. ([содержится в структуре шаблона](#Получение-списка-шаблонов))
    - `AllowRuntimeChange` - настройка, позволяющая применять изменения к шаблонам, которые проигрываются в данный момент. Тип `Boolean`. Если `true` - то изменения будут применяться к проигрываемому шаблону. Если `false` - то изменения будут применяться только к шаблону, который не проигрывается.
    - `Comment` - комментарий. Тип `String`. Необходим для расширенного описания действия элемента.
    - `Variables` - список переменных шаблона.
      - `Variable` (атрибут `Value`) - значение переменной шаблона.
...
1. В ответ вы получите сообщение-уведомление вида:
    ...
    <Command CmdGroup="Notifications" CmdName="EventChanged">
      <PlaylistId>fb36f71e-8ab5-4c10-a82d-1eba0df9659b</PlaylistId>
      <EventId>dae0a929-9a57-4e4b-985d-2eb3c04caf3a</EventId>
    </Command>
    ```

    где:

    - `PlaylistId` - идентификатор плейлиста, где был изменён элемент. Тип `Guid`. Необходим для [обновления содержимого плейлиста](#Получение-полного-содержимого-плейлиста).
    - `EventId` - идентификатор изменённого элемента. Тип `Guid`. Необходим для [обновления содержимого элемента](#Получение-содержимого-элемента).
...
1. Для этого [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:
    ...
    ```xml
    <Command CmdGroup="Playlists" CmdName="DeleteEvent" MessageId="69">
      <EventId>dae0a929-9a57-4e4b-985d-2eb3c04caf3a</EventId>
    </Command>
    ```
    ...
    - `EventId` - идентификатор элемента, который нужно удалить. Тип `Guid`.
...
1. В ответ вы получите сообщение-уведомление вида:
    ...
    - `PlaylistId` - идентификатор плейлиста, где была удалён элемент. Тип `Guid`. Необходим для [обновления содержимого плейлиста](#Получение-полного-содержимого-плейлиста).
    - `ScenarioId` - идентификатор истории, где была удалён элемент. Тип `Guid`. Необходим для [обновления содержимого истории](#Получение-содержимого-истории).

1. [Обновите содержимое истории](#Получение-содержимого-истории).

## Блокировка возможности редактирования переменной элемента

1. Для этого [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="LockEventField" MessageId="84">
      <EventId>ad09440b-f272-4898-942e-2b05415c1c41</EventId>
      <FieldId>30b3c78a-d1d5-4573-853a-16e83dcf7d1f</FieldId>
    </Command>
    ```

    где:

    - `EventId` - идентификатор элемента, переменную которого нужно заблокировать. Тип `Guid`.
    - `FieldId` - идентификатор переменной, которую нужно заблокировать. Тип `Guid`.
...
1. Приведите параметр `Locked` элемента в соответствие с полученным сообщением.

## Разблокировка возможности редактирования переменной элемента

1. Для этого [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="UnlockEventField" MessageId="96">
      <EventId>ad09440b-f272-4898-942e-2b05415c1c41</EventId>
      <FieldId>30b3c78a-d1d5-4573-853a-16e83dcf7d1f</FieldId>
    </Command>
    ```
    ...
    - `EventId` - идентификатор элемента, переменную которого нужно разблокировать. Тип `Guid`.
    - `FieldId` - идентификатор переменной, которую нужно разблокировать. Тип `Guid`.
...
## Группировка элементов внутри истории

Группировка необходима для одновременного запуска нескольких элементов внутри истории.

1. Для этого [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="GroupEvents" MessageId="111">
      <EventsId>
        <Id>fde59aa3-7121-454d-8c41-457834c9b4e4</Id>
        <Id>ad09440b-f272-4898-942e-2b05415c1c41</Id>
      </EventsId>
    </Command>
    ```
    ...
    - `EventsId` - список идентификатор элементов, которые необходимо объединить в одну группу.
    - `Id` - идентификатор конкретного элемента. Тип `Guid`.

1. В ответ вы получите следующие сообщения-уведомления вида:

    2.1. Изменение истории:
    ...
    - `PlaylistId` - идентификатор плейлиста, где был произведена группировка элементов. Тип `Guid`. Необходим для [обновления содержимого плейлиста](#Получение-полного-содержимого-плейлиста).
    - `ScenarioId` - идентификатор истории, где был произведена группировка элементов. Тип `Guid`. Необходим для [обновления содержимого истории](#Получение-содержимого-истории).

    2.2. Изменение каждого элемента, участвующего в группировке (количество сообщений будет равно количеству элементов, участвующих в группировке).
    ...
    - `EventId` - идентификатор элемента, который был сгруппирован. Тип `Guid`. Необходим для [обновления содержимого элемента](#Получение-полного-содержимого-элемента).

## Разгруппировка элементов внутри истории

1. Выберите элемент, который принадлежит к группе, которую необходимо разгруппировать.

1. Для этого [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="UngroupEvents" MessageId="155" GroupId="40e93334-fc13-4bb1-a5d7-a4f7cacf8526" />
    ```

    где:

    - `GroupId` - идентификатор группы, которую необходимо разгруппировать. Тип `Guid`. (находится в параметре `EventGroupId` элемента).

1. В ответ вы получите следующие сообщения-уведомления вида:

    Изменение каждого элемента, участвовавшего в группировке (количество сообщений будет равно количеству элементов, участвовавших в группировке).
    ...
    - `EventId` - идентификатор элемента, который был разгруппирован. Тип `Guid`. Необходим для [обновления содержимого элемента](#Получение-полного-содержимого-элемента).

## Подготовка элемента и шаблона к проигрыванию

Перед проигрыванием шаблона необходимо прогрузить шаблон и элемент, запускающий его.

1. Для этого должны быть настроена схема проигрывания и запущен `Carrot Engine`.

1. [По соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="EnqueueEvent" MessageId="170">
      <EventId>ad09440b-f272-4898-942e-2b05415c1c41</EventId>
    </Command>
    ```
    ...
    - `EventId` - идентификатор элемента, который необходимо прогрузить. Тип `Guid`.

1. В ответ вы получите следующие сообщения-уведомления вида:

    Изменение статуса элемента - сначала `Loading`:
    ...
    - `PlaylistId` - идентификатор плейлиста, где был изменен элемент. Тип `Guid`.
    - `EventId` - идентификатор элемента, который был изменен. Тип `Guid`. Необходим для [обновления содержимого элемента](#Получение-полного-содержимого-элемента).
    - `Status` - статус элемента. Тип `Int`. Необходим для отображения состояния элемента и шаблона (Unloaded = 0, Loading = 1, Ready = 2, Active = 3).

## Проигрывание шаблона

1. [По соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="SetTemplateState" MessageId="172">
      <EventId>ad09440b-f272-4898-942e-2b05415c1c41</EventId>
    </Command>
    ```
    ...
    - `EventId` - идентификатор элемента, который необходимо запустить. Тип `Guid`.

1. В ответ вы получите сообщение-уведомление вида:

    Изменение статуса элемента `Active`:
...
## Завершение проигрывания шаблона

Есть шаблоны, у которых указано состояние, после которого его проигрывание в контейнере прекращается (Closing State).

1. Чтобы вызвать его, [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="TakeOut" MessageId="176">
      <EventId>ad09440b-f272-4898-942e-2b05415c1c41</EventId>
    </Command>
    ```
    ...
    - `EventId` - идентификатор элемента, который сейчас запущен. Тип `Guid`.

1. В ответ вы получите следующие сообщения-уведомления вида:

    Изменение статуса элемента `Ready`:
...
## Остановка всех проигрываемых шаблонов внутри плейлиста

В процессе проигрывания шаблонов может возникнуть необходимость оперативно остановить все проигрываемые шаблоны и убрать их с показа.

1. Для этого [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="StopPlaylist" MessageId="285" UserId="ac4eb08d-7d7a-47e1-96da-b26a304a60de">
      <PlaylistId>fb36f71e-8ab5-4c10-a82d-1eba0df9659b</PlaylistId>
    </Command>
    ```
    ...
    - `UserId` - идентификатор учетной записи. Тип `Guid`. [Находится в ответном сообщении при авторизации](authorization.md)
    - `PlaylistId` - идентификатор плейлиста, который необходимо остановить. Тип `Guid`.

1. В ответ вы получите следующие сообщения-уведомления вида:

    Изменение каждого элемента, который был запущен (количество сообщений будет равно количеству запущенных элементов).
...
Прогруженные шаблоны занимают оперативную и видеопамять. Могут возникнуть ситуации, когда для следующего шаблона памяти не хватит для загрузки или правильной работы.

В таких случаях рекомендуется выгружать шаблоны, которые уже проигрались и не будут запускаться далее в плейлисте.

1. Для этого [остановите все запущенные элементы](#Завершение-проигрывания-шаблона), относящиеся к шаблону, который необходимо выгрузить.

1. [по соединению для обмена сообщениями](connection.md) отправьте на сервер сообщение вида:

    ```xml
    <Command CmdGroup="Playlists" CmdName="UnloadEvent" MessageId="289">
      <EventId>ad09440b-f272-4898-942e-2b05415c1c41</EventId>
    </Command>
    ```
    ...
    - `EventId` - идентификатор элемента, относящегося к шаблону, который необходимо выгрузить. Тип `Guid`.

    > Произойдет выгрузка не только шаблона, но и всех элементов, относящихся к нему.

1. В ответ вы получите следующие сообщения-уведомления вида:

    Изменение статуса элемента на `Unloaded` (количество сообщений будет равно количеству запущенных элементов).
